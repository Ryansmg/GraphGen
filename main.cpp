...
#define ssize(v) ( (long long) v.size() )
...
protected:
    ...
    static void chk_(bool b, const string& msg, int line) {
        if(!b) {
            cerr << "Assertion failed at line " << line << ":\n";
            cerr << msg << '\n';
            exit(1);
        }
    }
...
public:
    ...
    const vector<int>& node_group() {
        forf(i, 1, n) group[i] = find_group(i);
        return group;
    }
    ...
    vector<vector<int>> connected_components() {
        ...
        forf(i, 1, n) ret1[find_group(i)].push_back(i);
    ...
    bool is_tree() { return ssize(edges) == n - 1 && ssize(connected_components()) == 1; }
    ...
    static Graph tree(const int n, const int linearRatio) {
        Graph ret(n);
        vector<int> p(n, 0);
        forf(i, 0, n - 1) if (i > 0) p[i] = rnd.wnext(i, linearRatio);
        vector<int> perm(n, 0); forf(i, 0, n - 1) perm[i] = i;
        shuffle(perm.begin() + 1, perm.end());
        forf(i, 1, n - 1) ret.add_edge(perm[i] + 1, perm[p[i]] + 1);
        assert(ret.is_tree());
        return ret;
    }
    ...
    static Graph tree_without_degree_of_2(const int n, const Args&... args) {
        chk(n >= 4, "must have at least four nodes");
    ...
    static Graph cactus(const int n, const int tree_n, const int cycle_cnt, const int linearRatio = -1) {
        ...
        forf(i, 1, tree_n) {
            ...
            if(contains(cycleNodes, i)) {
                ...
                forf(j, 1, nsz) nextNodes[i].push_back(curNodeNum++);

                forn(j, nsz) ret.add_edge(nextNodes[i][j], nextNodes[i][j+1]);
    ...
    static Graph line_graph(const int n, const int first_node = -1, const int last_node = -1) {
        ...
        if(first_node != -1) {
            ...
            forn(i, n) if(arr[i] == first_node) swap(arr[0], arr[i]);
        }

        if(last_node != -1) {
            chk(1 <= last_node && last_node <= n, "last_node is not a valid node");
            forn(i, n) if(arr[i] == last_node) swap(arr[n-1], arr[i]);
        }
        ...
        Graph ret(n);
        forn(i, n-1) ret.add_edge(arr[i], arr[i+1]);
        return ret;
    }

    static Graph complete_graph(const int n) {
        Graph ret(n);
        forf(i, 1, n) forf(j, i+1, n) ret.add_edge(i, j);
        return ret;
    }

    static Graph star_graph(const int n, const int root = 1) {
        Graph ret(n);
        forf(i, 1, n) if(i != root) ret.add_edge(root, i);
        assert(ret.is_tree());
        return ret;
    }
    ...
    static Graph skeleton_graph(const int n) {
        ...
        int line_len = n / 2 + 1;
        Graph g(n);
        forf(i, 2, line_len) g.add_edge(i - 1, i);
        forf(i, 2, line_len - 1) g.add_edge(i, line_len + i - 1);
        return g;
    }
...
public:
    ...
    explicit Tree(const Graph& g) : Graph(g) { chk(this->is_tree(), "g must be tree"); }
    explicit Tree(Graph&& g) : Graph(std::move(g)) { chk(this->is_tree(), "g must be tree"); }

    static Tree tree(const int n) { return Tree(Graph::tree(n)); }
    static Tree tree(const int n, const int linearRatio) { return Tree(Graph::tree(n, linearRatio)); }

    template <typename... Args>
    static Tree skeleton_graph(int n, const Args&... args) { return Tree(Graph::skeleton_graph(n, args...)); }
};
...
int main(int argc, char** argv) {
    ...
    int n; cin >> n; cout << n << "\n";
    Tree::skeleton_graph(n).print_edges();
    Tree::tree(10).print_edges();
}
